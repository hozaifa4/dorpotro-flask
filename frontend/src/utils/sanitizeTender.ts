...
export function sanitizeTenderRecord(raw: any): Tender {
  ...
  // 7. Sanitize clean description
  const cleanPackageDesc = getCleanTenderDescription(raw.packageDescription, raw.packageNo, raw.briefDescription);

  return {
    ...raw,
    id,
    packageDescription: cleanPackageDesc,
    publicationDate,
...
/**
 * Cleans leading package code prefixes if present so the UI displays the full, intact work description.
 */
export function getCleanTenderDescription(packageDesc?: string, packageNo?: string, briefDesc?: string): string {
  ...
  if (!rawDesc || rawDesc === 'null' || rawDesc === 'undefined') return 'N/A';

  // 1. If packageNo is at the start, strip only the packageNo prefix
  ...
  // 2. Strip standard slash package code prefixes (e.g. "e-Tender/LGED/2026/01 ") if present
  let cleaned = rawDesc
    .replace(/^(?:e\s*[-–]\s*)?[A-Za-z0-9_.\-/#()]*(?:\/[A-Za-z0-9_.\-/#()]+)+(?:\s+Lot[-_\s]*\d+)?\s+/i, '')
    .replace(/^["'\s\-:;,.]+/, '')
    .replace(/["']+$/, '');

  return (cleaned && cleaned.length > 3) ? cleaned : rawDesc;
}
